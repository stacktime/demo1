...
/**
 ...
 @GetMapping("/findDeviceById") public Object findDeviceById(@RequestParam Integer id) {
 if (null == id) {
 throw new RuntimeException("id不能为空");
 }
 ...
 @GetMapping("/modifyStatus") public Object modifyStatus(@RequestParam Integer id, @RequestParam Integer status) {
 ...
 @GetMapping("/insertStatus") public Object insertStatus(@RequestParam String code, String name) {
 deviceService.insertStatus(code, name);
 return 0;
 }

 @PostMapping("/saveDevice") public Object saveDevice(@RequestBody RequestDTO request) {
 if (null == request) {
